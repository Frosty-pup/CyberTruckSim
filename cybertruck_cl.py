...
class CyberTruckSimulator:
    def __init__(self):
        ...
        self.obstacles = ["roadblock", "traffic jam", "bad weather", "wildlife crossing"]
    ...
    def drive(self):
        if self.is_running and not self.failure_occurred:
            if self.battery_level > 0:
                distance_this_drive = random.randint(1, 10)  # Drive between 1 and 10 miles
    ...
    def visit_service_center(self):
        ...
        service_center_time = random.randint(5, 10)  # Simulate a long delay in hours
        time.sleep(2)  # Simulate the time passing (shortened for playability)
        print(f"Spent {service_center_time} hours at the service center.")
    ...
    def fix_issue(self):
        if self.failure_occurred:
            if "battery dead" in self.issues and self.battery_level <= 0:
                ...
                print("\nFixing the issue... This may take some time.")
                time.sleep(2)
                print("The issue has been fixed. You can start the truck again.")
                self.failure_occurred = False
        else:
    ...
    def trigger_obstacle(self):
        ...
        if obstacle == "roadblock":
            ...
            self.distance -= random.randint(1, 5)  # Set back distance by a few miles (as a detour)
        elif obstacle == "traffic jam":
            print("You're stuck in a traffic jam! Progress is slower for the next few miles.")
            # Drive fewer miles next time
            ...
            self.battery_depletion_rate += 5  # Battery depletes faster due to weather
        ...
        # Reset battery depletion rate after an obstacle event (if applicable)
        self.battery_depletion_rate = 5
